...
function updateCategories(products) {
	...
	products.forEach((product, id) => {
		var category = product.cat//.toLowerCase();
		if (category in categoriesArray) {
			categoriesArray[category].count += 1;

			if (product.category in categoriesArray[category].categories) {
				categoriesArray[category].categories[product.category].count += 1
			} else {
				categoriesArray[category].categories[product.category] = {
					count: 1,
				};
			}
		} else {
	...
	// categoriesArray = categoriesArray.sort()
...
function createCategories(categoriesArray) {
	var categoriesContainer = document.getElementById('categories-array');
	while (categoriesContainer.firstChild) {
	    categoriesContainer.removeChild(categoriesContainer.firstChild);
	}

	var subCategoriesContainer = document.getElementById('categories-sub-array');
	while (subCategoriesContainer.firstChild) {
	    subCategoriesContainer.removeChild(subCategoriesContainer.firstChild);
	}

	// option.value = "reset";
	// option.textContent = "<?= $texts['OrderPage.categories_all']?>";

	var categorySpan = document.createElement('span');
    categorySpan.className = "category-item category-selected";
    categorySpan.dataset.name = category;
    var selectedTextNode = document.createTextNode("All Products"); 
	categorySpan.appendChild(selectedTextNode);
	categorySpan.onclick = function (e) {
		// showSubCats(categoriesArray[this.dataset.name].categories)
		$('.category-item').removeClass('category-selected');
		$(this).addClass('category-selected');
		$('#productsList').bootstrapTable('filterBy', "");
		showSubCats("","");
	...
	for (var category in categoriesArray){
	    if (typeof categoriesArray[category] !== 'function') {
		...
		/*	var option;
		    ...
		    option.textContent = capitalizeFirstLetter(category)+'('+categoriesArray[category]+')';
	...
	*/
	        ...
	        categorySpan.dataset.name = category;
	        var selectedTextNode = document.createTextNode(capitalizeFirstLetter(category)+'('+categoriesArray[category].count+')'); 
			categorySpan.appendChild(selectedTextNode);
			categorySpan.onclick = function (e) {
				$('.category-item').removeClass('category-selected');
				$(this).addClass('category-selected');
				$('#productsList').bootstrapTable('filterBy', {cat: this.dataset.name});
				showSubCats(categoriesArray[this.dataset.name].categories, this.dataset.name)
			}
	        categoriesContainer.appendChild(categorySpan);
	    }
	}
...
function showSubCats(cats, cName) {
	var subCatsContainer = document.getElementById('categories-sub-array');
	while (subCatsContainer.firstChild) {
	    subCatsContainer.removeChild(subCatsContainer.firstChild);
	}
	...
	var categorySpan = document.createElement('span');
    categorySpan.className = "category-item subcategory category-selected";
    categorySpan.dataset.name = cName;
    var selectedTextNode = document.createTextNode("All " + cName + " products"); 
	categorySpan.appendChild(selectedTextNode);
	categorySpan.onclick = function (e) {
		// showSubCats(categoriesArray[this.dataset.name].categories)
		$('.category-item').removeClass('category-selected');
    ...
    subCatsContainer.appendChild(categorySpan);

	for (var category in cats){
	    if (typeof cats[category] !== 'function') {
	
	        var categorySpan = document.createElement('span');
	        categorySpan.className = "category-item subcategory";
	        categorySpan.dataset.name = category;
	        var selectedTextNode = document.createTextNode(capitalizeFirstLetter(category.toLowerCase())+'('+cats[category].count+')'); 
			categorySpan.appendChild(selectedTextNode);
			categorySpan.onclick = function (e) {
				$('#categories-sub-array').find('.category-item').removeClass('category-selected');
				$(this).addClass('category-selected');
				$('#productsList').bootstrapTable('filterBy', {category: this.dataset.name});
				console.log(e)
			}
	        subCatsContainer.appendChild(categorySpan);
	    }
...
function handleClickCategory() {
	var categoryDivs = document.getElementsByClassName('category-item')
	categoryDivs.onClick = function(e) {
		console.log(e);
		alert()
	}
}
...
$(document).ready(function() {
	...
	var orderItemId = 0;
	updatePricelistTable(getPricelist('lt'));
	...
	$('#sendOrderForm').click(function(event) {
		...
		if (email == "") {
			...
			$('#orderEmail').css({'border':'1px solid #c0c0c0'});
	...
	<section id="orderOnline">
		<div class="container container-order">
			...
			<div class="row">
				...
				<div class="col-lg-5">
					<p>
						<?= $texts['OrderPage.description2']; ?>
					</p>
					<p>
						<img class="order-icon"  src="img/name.png" alt="<?= $texts['Contact.name']; ?>" />
						<input type="text" name="name" id="orderName"  placeholder="<?= $texts['Contact.name']; ?>" />
					</p>
					<p>
						<img class="order-icon"  src="img/shop/email.png" alt="<?= $texts['Contact.email']; ?>" />
						<input type="text" name="email" id="orderEmail"  placeholder="<?= $texts['Contact.email']; ?>" 
						<?php
						if (isSet($_GET['email'])) {
					...
					<p>
						...
						<input type="text" name="address" id="orderAddress"  placeholder="<?= $texts['Contact.address']; ?>" />
					</p>
					<p>
						<img class="order-icon"  src="img/shop/language.png" alt="<?= $texts['Contact.city']; ?>" />
						<input type="text" name="city" id="orderCity"  placeholder="<?= $texts['Contact.city']; ?>" />
					</p>
					<p>
						<img class="order-icon"  src="img/shop/phone.png" alt="<?= $texts['Contact.phone']; ?>" />
						<input type="text" name="phone" id="orderPhone"  placeholder="<?= $texts['Contact.phone']; ?>" />
					</p>
					<p>
						<textarea name="message" id="orderMessage"  placeholder="<?= $texts['Contact.note']; ?>" /><?= $texts['Contact.note']; ?>
						</textarea>
					</p>
					<p>
						<?= $texts['OrderPage.description3']; ?>
					</p>
				</div>
			</div>
			<div class="row">
				<div class="col-lg-12">
					<ul class="pricelists">
					<p style="display:inline-block;"><?= $texts['OrderPage.selected_pricelist2']; ?>
					</p>
					<?php 
					$pricelistHtml = '';
					$counter = 0;
				...
				<div class="col-lg-12">
					<hr/>
					<!-- <p style="text-transform:none;"><?= $texts['OrderPage.category_title']; ?> -->
					<p style="display:inline-block;"><?= $texts['OrderPage.category_name']; ?></p>
					<span id="categories-array"></span>
					<hr/>
					<p style="display:inline-block;"><?= $texts['OrderPage.sub_category_name']; ?></p>
					<span id="categories-sub-array"></span>
				</div>
			</div>
			<div class="row">
				<div class="col-lg-9">
					...
					<div style="">
						...
						<form id="orderForm">
							...
							<p style="margin-top:20px;font-size: 16px;">
								<button class="btn btn-primary" id="sendOrderForm" type="submit"><?= $texts['OrderPage.submit']; ?></button>
