...
$(document).ready(function() {
	...
	$('#subscribePanelButton').hide()
	$(window).on('scroll', function() {
	    var scrollTop = $(this).scrollTop();

	    if (scrollTop < 145) {
	    	if (!$('#subscribePanel').hasClass('subscribe-top') && !$('#subscribePanel').hasClass('hide-panel')) {
	    		$('#subscribePanel').addClass('subscribe-top')
	    		$('#subscribePanelButton').hide()
	    	}
	    } else {
			if ($('#subscribePanel').hasClass('subscribe-top')) {
	    		$('#subscribePanel').removeClass('subscribe-top')
	    		$('#subscribePanelButton').show()
	    	}
	    }
...
.subscrite-top #subscribeEmail {
	width:30% !important;
	float:left !important;
}

.subscribe-top .first-input {
	width: 30% !important;
	float: left;
}

.subscribe-top .second-input {
	max-width: 130px !important;
	width: initial;
	float: left;
}

.subscribe-top .third-input {
	max-width: 130px !important;
	margin-top: -10px;
	width: 200px;
	float: left;
}
.subscribe-top .fourth-input {
	max-width: 130px !important;
	margin-top: -10px;
	width: initial;
	float: left;
}
.subscribe-top .fifth-input {
	max-width: 130px !important;
	margin-top: -12px;
	width: initial;
	float: left;
...
<div id="subscribePanel" class="hidden-subscribe subscribe-top" style="<?php
if (!isSet($_GET['test'])) {
	echo "display:none";
}
?>">
