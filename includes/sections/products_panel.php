...
function showProducts($products, $type, &$languageSelected, &$texts) {
	...
	if (is_array($products) && count($products) > 0) {
		$tempHtml = '
		<div id="'.$type.'Products" class="row list-group">';
	
		foreach ($products as $promo) {
			$tempProduct = '
			<div class="item productItem">
				<a href="' . $promo['image_link'] . '" data-featherlight="image">
				<img class="group list-group-image-products" src="' . $promo['image_link'] . '" alt="" />
				</a>
				<div class="col-xs-12 col-md-12">
					...
					<p>
						<span class="price-new products-' . $type . ' list-group-item-heading">
...
<img src="images/arrow_left.png" alt="" id="arrow-left" style="display:none"/>
	...
	<div id="productsPanelTitle" class="">
		...
		$charsArray = mbStringToArray($texts['ProductsPanel.Title']);
		for($i = 0; $i < count($charsArray); $i++) {
			echo "$charsArray[$i]<br/>";
		}
		?>
	</div>
</div>
